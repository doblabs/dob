...
def tab_complete(controller):
    ...
    def _do_complete():
        try:
            ...
            controller.client_logger.error(_('Expected environ "COMP_WORDS" not set.'))
            sys.exit(1)
        try:
...
def _choices_datetimes(controller, incomplete, time_hint, parser_err):
    ...
    friendly_from_hints = [
        ...
        "3_years_ago",
        "Monday_at_16:44",
        "at_noon" "1_week_ago_at_midnight",
        # Hahaha, futuristic!
    ...
    if incomplete:
        choices = [ch for ch in choices if not incomplete or ch.startswith(incomplete)]
    return choices
...
def choices_activities(
    ...
    choices = [
        "{}@{}".format(act.name, act.category.name if act.category else "")
